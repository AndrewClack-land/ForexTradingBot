...
def _dump_trade(tr: Any, symbol: str) -> dict:
    return {
        ...
        "broker_closed": bool(getattr(tr, "broker_closed", False)),

        # Position-adding identity. Add-on entries are nested inside their primary
        # trade so an interrupted pyramid is restored with each entry's own
        # tickets, stop and break-even state intact.
        "idea_id": getattr(tr, "idea_id", None),
        "entry_index": int(getattr(tr, "entry_index", 1) or 1),
        "idea_trigger_signatures": [
            str(s) for s in (getattr(tr, "idea_trigger_signatures", []) or [])
        ],
        "be_for_idea_risk": bool(getattr(tr, "be_for_idea_risk", False)),
        "addons": [
            _dump_trade(addon, symbol)
            for addon in (getattr(tr, "addons", []) or [])
...
def save_active_trades(active_trades: Dict[str, Any], path: Path) -> None:
    ...
    for symbol, tr in (active_trades or {}).items():
        if tr is None:
            continue
        payload[symbol] = _dump_trade(tr, symbol)
...
def _load_trade(d: dict, symbol: str) -> Any:
    ...
    tr.broker_closed = bool(d.get("broker_closed", False))

    tr.idea_id = d.get("idea_id")
    tr.entry_index = int(d.get("entry_index", 1) or 1)
    tr.idea_trigger_signatures = [
        str(s) for s in (d.get("idea_trigger_signatures") or [])
    ]
    tr.be_for_idea_risk = bool(d.get("be_for_idea_risk", False))
    addons: List[Any] = []
    for raw_addon in (d.get("addons") or []):
        if not isinstance(raw_addon, dict):
            continue
        try:
            addons.append(_load_trade(raw_addon, symbol))
        except Exception:
            # A broken add-on row must not cost us the primary entry, whose
            # tickets are what keeps the live position managed.
            continue
    tr.addons = addons
    return tr
...
def load_active_trades(path: Path) -> Dict[str, Any]:
    ...
    for symbol, d in (data or {}).items():
        try:
            restored[symbol] = _load_trade(d, symbol)
        except Exception:
            continue
